...
func newCenciWatchTestBoard(t *testing.T, watcher cenciwatch.Watcher) Board {
	...
	return NewBoard(p, nil, nil, nil, "", "", "", 0, 0, "Working", false, false, watcher, nil, true)
}
...
func newCenciWatchCardTestBoard(t *testing.T, cardNumber int, cardTitle string, sessionMaxLen int) Board {
	t.Helper()
	p := provider.NewFakeProvider()
	b := NewBoard(p, nil, nil, nil, "", "", "", sessionMaxLen, 0, "Working", false, false, nil, nil, true)
...
func newAgentCountsBoard(t *testing.T, cards []provider.Card) Board {
	t.Helper()
	p := provider.NewFakeProvider()
	b := NewBoard(p, nil, nil, nil, "", "", "", config.DefaultSessionMaxLength, 0, "Working", false, false, nil, nil, true)
	msg := boardFetchedMsg{board: provider.Board{
...
func TestBoard_DispatchStatusSegment_AppearsThenClearsLiveViaWatcher(t *testing.T) {
	...
	p := provider.NewFakeProvider()
	b := NewBoard(p, nil, nil, nil, "", "", "", 0, 0, "Working", false, false, fw, nil, true)
	b.Width = 120
	b.Height = 40
