...
When a code comment describes a defensive guard or fallback behavior that the plan names as a mitigation (e.g., a doc string explaining what an `if !ok { continue }` branch does), verify the conditional logic actually implements the guard as described. A comment-code mismatch can pass tests if the guarded path is unreachable in practice (upstream invariants prevent it) but will mislead reviewers and create maintenance debt. Diff the comment's claim against the conditional: does a claimed "omit this entry" map to an actual `continue` or filter-out? Does "apply the override only if it's a BindingAction" map to an `if override.Kind != BindingAction { continue }`? A guard described but not coded is a trap reviewers can miss even when reading the code carefully.

## Sibling Code Path Auditing for Output-Shape Bugs
When a bounds-safety, truncation, or rendering bug is reported against one code path that produces a particular output form, audit the entire function for other code paths that produce *identical* output (even if they reach it via different routes — early returns, fallback branches, degenerate cases). A fix applied to only the reported path leaves the bug half-fixed in its sibling paths. Example: `blockedByLine` (#631) had two syntactically distinct code paths (an early `budget <= 0` guard-clause return at line 752, and a loop-exhausted fallback at line 760) both computing `composeBlockedLine(nil, count)` — the same degenerate `"<glyph> +N"` form. The initial fix wrapped only the fallback in `truncateCell()`; a second review round found the early-return path still untruncated, requiring a symmetric fix. Fixing only one instance and not auditing for others cost two extra review-and-fix cycles. When implementing or reviewing a fix that hardens one output path, grep the function for all sites that call the same output-building helper (`composeBlockedLine`, `composeDetailMarkdown`, etc.) or produce output of the same structural category (degenerate forms, common label patterns, identical format strings), and ensure the same hardening (truncation, sanitization, bounds check) is applied everywhere that output can be reached.
...
## CLI Verb Dispatch Tests
When implementing a CLI verb across multiple commits (functions in Commit 1, `main()` dispatch in Commit 2) or when dispatch delegation is separate from function delegation, add a subprocess test that builds the real binary and verifies the verb actually routes to the feature implementation. Standalone unit tests of the verb functions are insufficient — they can pass while the dispatch call site in `main()` is completely missing. Missing `main()` wiring is invisible to `go build`/`go test ./...` (both pass with no syntax errors) and to diff reviews (missing code doesn't appear in diffs); it only surfaces during manual testing or external review. Example: `main_trust_dispatch_test.go` uses `os/exec` to build `lazyboards` and run it as a subprocess, verifying the compiled binary's `main()` actually routes `lazyboards trust` to `runTrustVerb` — a gap that unit tests and `go test ./...` both missed.
