...
- **Fix**: After the `err != nil` check, added `if snap == nil { return cenciWatchErrorMsg{err: errors.New(...)} }` so a nil result routes through the same capped-backoff reconnect machinery as a real error.
