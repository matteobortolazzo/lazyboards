...
### bubbles/textarea viewport reset requires View() then Update(), not SetValue() alone
- **Date**: 2026-02-24
- **Ticket**: #223
- **What happened**: Attempted to reset textarea viewport scroll offset after resizing by calling `SetValue(Value())`, expecting it to call `Reset()` which would trigger `viewport.GotoTop()`. However, the viewport scroll offset remained unchanged. Analysis revealed that while `SetValue()` does call `Reset()`, it never triggers `repositionView()`, which is the only function that updates the viewport's internal scroll position.
- **Root cause**: The textarea component's `repositionView()` method is unexported and only called during `Update()`. Even `Update(nil)` alone is insufficient because `repositionView()` uses the viewport's `lines` field (which records rendered content lines) to compute scroll offsets. The `viewport.lines` field is only populated by `viewport.SetContent()`, which is only called inside `View()`. So the state mutation sequence is: `View()` → `SetContent()` populates `viewport.lines` → `Update(nil)` → `repositionView()` can now correctly compute scroll offsets.
- **Fix**: Changed `recalcCreateInputs()` to call `View()` first (to populate viewport.lines via SetContent), then `Update(nil)` (to trigger repositionView). Pattern: `m.createInput.View(); m.createInput.Update(nil)`. Do not rely on `SetValue()` or `Reset()` alone for viewport position changes.
- **Rule**: When resetting a bubbles/textarea viewport scroll position after a geometry change (resize, height adjustment), always call `View()` to update the viewport's internal line state, followed by `Update(nil)` to trigger cursor repositioning. Using `SetValue(Value())` alone does not reset scroll because the internal `repositionView()` call is missing. Understand that BubbleTea component state (especially viewport content lines) is lazy-initialized and may only be populated during `View()`, not during `Update()`.

### Cursor index must be consistent between bounds-checking and card resolution in filtered views
- **Date**: 2026-03-19
- **Ticket**: #234
- **What happened**: Global filtering was implemented to reduce the visible card list. Navigation bounds correctly clamped `col.Cursor` to `len(filteredCards())-1`, but ALL action dispatch sites (`e` edit, `o` open ticket, `p` open PR, custom actions, comment mode, mouse scroll, mouse click) still used `col.Cards[col.Cursor]` (raw unfiltered list) instead of `filteredCards()[col.Cursor]`. When a filter was active, every card-scope operation silently acted on the wrong card.
- **Root cause**: The cursor was treated as an index into the filtered list for navigation (bounds checking) but as an index into the raw list for action dispatch. These two interpretations are incompatible. The bounds check (navigation) passed, so tests didn't catch the mismatch — tests only verified that cursor stayed in-bounds, not that the correct card was acted upon.
- **Fix**: Added a `selectedCard()` helper method that resolves `filteredCards()[col.Cursor]` when a filter is active, `col.Cards[col.Cursor]` otherwise. Replaced all 10 occurrences of direct `col.Cards[col.Cursor]` access at action dispatch sites with `selectedCard()`. Also fixed `handleMouseScroll` bounds for WheelDown and `handleCardClick` iteration to use filtered cards when a filter is active.
- **Rule**: When introducing a filtered/virtual view of a list that coexists with cursor-based navigation, ensure the cursor index interpretation is consistent across ALL consumers (navigation bounds, action dispatch, mouse hit detection, scroll bounds). Create a helper method that centralizes card resolution (`selectedCard()`) and use it everywhere instead of direct array indexing. Never mix `filteredCards()[cursor]` in one place and `rawCards[cursor]` in another — the inconsistency will cause silent failures in features that use both filtering and actions.

### A blocking-read interface returning (nil, nil) must be treated as an error in a self-chaining Cmd
- **Date**: 2026-07-11
- **Ticket**: #257
- **What happened**: `subscribeAgentWatchCmd` mapped `watcher.ReadNext()` results as: non-nil error → `agentWatchErrorMsg`, else → `agentSnapshotMsg{snapshot: snap}`. But a `Watcher` can legitimately return `(nil, nil)` — no snapshot and no error (a clean socket close, or the `FakeWatcher` exhausting its scripted results). That nil snapshot was stored as a "successful" read, which reset the reconnect backoff to its initial value and immediately re-subscribed. Because the self-chaining Cmd re-fires on every success, a persistent `(nil, nil)` source spins the subscription at full speed with no backoff. Two reviewers flagged it; the real `socketWatcher` returns `net.ErrClosed` (an error) on close so it could not spin today, but the interface contract left the door open.
- **Root cause**: A blocking-read method with signature `(*T, error)` has three outcomes, not two: data, error, and the ambiguous "no data, no error". Mapping only on `err != nil` silently funnels the ambiguous case into the success path. In a self-chaining `tea.Cmd` the success path has no rate limit, so the ambiguity becomes a busy loop.
- **Fix**: After the `err != nil` check, added `if snap == nil { return agentWatchErrorMsg{err: errors.New(...)} }` so a nil result routes through the same capped-backoff reconnect machinery as a real error.
- **Rule**: When a `tea.Cmd` re-subscribes to a blocking-read interface whose method returns `(*T, error)`, treat a `(nil, nil)` result as an error, not a success. The success branch of a self-chaining Cmd re-fires with no delay, so any non-erroring "empty" result there becomes an unthrottled spin loop. Guard `result == nil` explicitly; never assume `err == nil` implies a usable value.


### Don't distort production code to satisfy a test that asserts an implausible shape — fix the test
- **Date**: 2026-07-12
- **Ticket**: #283
- **What happened**: A test (`TestDispatch_HandleEnrollMsg_SuccessRequeriesStatus`) sent a `dispatchEnrollMsg{}` success directly into `Update()` and then asserted `len(fe.RunShellOutputCalls) == 2`, even though its own comment said the enroll call was "out of scope here". Since the success handler's single `queryDispatchStatusCmd` re-query only issues one `RunShellOutput` call, the assertion was unsatisfiable by correct code. The implementer made it pass by returning `tea.Batch(queryDispatchStatusCmd(...), queryDispatchStatusCmd(...))` — firing two identical `agentwatch dispatch status` subprocesses on every enroll — and wrote a bogus "guard against a race" comment. `tea.Batch` runs both concurrently, so it doesn't even sequence a before/after read; it was pure test-appeasement that introduced a real double-subprocess bug.
- **Root cause**: When a test forces production into an implausible shape (a duplicated call, an unreachable branch, a magic constant), the default reaction of "make the test green" distorts production instead of questioning the test. The approved plan explicitly specified a single re-query; the test contradicted the plan and was simply wrong.
- **Fix**: Corrected the test to assert exactly one `RunShellOutput` call (the single status re-query) and reverted the handler to `return b, queryDispatchStatusCmd(b.executor)`.
- **Rule**: If making a test pass requires production code to do something the approved plan/behavior does not call for — especially duplicating a side-effecting call or adding an unexplained `tea.Batch` of identical Cmds — stop and fix the test, do not bend production to it. A "race guard" comment on two concurrent `tea.Batch` Cmds is a red flag: batched Cmds run concurrently and cannot order a write-then-read. Also see testing.md ("NEVER assert call counts"): where a count assertion is genuinely guarding observable behavior (e.g. "no duplicate subprocess"), keep it minimal (`== 1`) and comment why.


### Cross-service status/enum strings must be pinned to the producer's real wire value, not a doc comment
- **Date**: 2026-07-14
- **What happened**: agentwatch integration silently dropped the `need-input` agent badge. The socket path (#316), config, protocol, and struct decode were all correct — snapshots streamed fine — but lazyboards compared the window status against the string constant `agentStatusNeedInput = "need_input"` (underscore) in `model.go` (plus two `case "need_input"` literals in `view.go`'s `agentStatusSymbol`/`agentBadgeStyle`). The daemon actually emits `"need-input"` (hyphen): its status is serialized from `detect.StatusNeedInput.String()`, which returns `"need-input"`. So every agent waiting for input got no badge, no status-bar count, and lost the active-window tiebreak. Only `need-input` was affected; `running`/`done`/`failed` matched by luck.
- **Root cause**: The value was copied from the daemon's own doc comment (`agentwatch/pkg/watch/snapshot.go` says `"... "need_input", or "failed""`) instead of the actual serialized token. The doc comment is wrong/stale relative to the `String()` method that produces the wire value. Worse, EVERY lazyboards test fixture also hardcoded `"need_input"`, so the suite was self-consistent with the bug and stayed green — a case of "copy expected values from implementation" (testing.md) hiding a real defect.
- **Fix**: Changed the three production spots to `"need-input"`, corrected all test fixtures, and added `TestBoard_AgentBadge_NeedInputFromDaemonWireFormat` which `json.Unmarshal`s a real daemon-format NDJSON line (status `"need-input"`) and asserts the badge/count — pinning the token to the producer's wire shape rather than a repeated literal. Verified it fails against the old underscore value (red) and passes after (green).
- **Rule**: When matching an enum/status string that crosses a service boundary (socket, API, IPC), pin the expected value to the PRODUCER's actual serialization (`String()` method, an observed wire sample), never to a doc comment or a value you also hardcode in your own tests. For such cross-boundary values, prefer a test that decodes a real wire sample over one that constructs the struct field by hand — otherwise producer and consumer tests can share the same wrong constant and both stay green. When a badge/count "doesn't show" but the transport is healthy, suspect a status-token mismatch before the plumbing.

### Bash working directory resets between calls in agent sandbox — chain worktree commands with &&
- **Date**: 2026-07-14
- **Ticket**: #357
- **What happened**: `go test ./internal/provider/...` appeared to fail with "no tests to run" and silently ran against the main repo instead of the feature worktree.
- **Root cause**: In the agent Bash environment, working directory resets between separate Bash tool calls. A standalone `cd <worktree-path>` in one Bash call does not persist to the next Bash call. Non-git build/test commands then run in the main repository directory with incorrect test package resolution.
- **Fix**: Chain commands within a single Bash call: `cd <worktree-path> && go test ./...` (not split into separate `cd` and `go test` calls). This ensures the test runs in the correct working directory.
- **Rule**: When running build/test/compile commands (go test, cargo, make, etc.) against code in a feature worktree through the agent Bash tool, always chain the `cd <path> &&` within the same call. Do not rely on `cd` persisting across separate Bash invocations. Unlike git commands (which can resolve the worktree context from `.git` files), build tools execute relative to the actual working directory.
