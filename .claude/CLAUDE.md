...
| `keymap_panels_test.go` | Git panel keymap routing (#511 PR 1/2): default-parity dispatch/items/hints against `config.DefaultGitActions()`, `keymaps.git_panel` override winning over a built-in, explicit unbind (no-op dispatch + menu row removal), the hint-bar<->dispatch invariant, multi-key-sequence handling (row kept, key label blanked), untrusted action-name sanitization in the menu view |
