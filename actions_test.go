...
func TestAction_IgnoredInLoadingMode(t *testing.T) {
	...
	b := NewBoard(p, actions, nil, nil, fe, "", "", "", 0, 0, 0, "Working", false, false, nil)
...
func TestTicketOpen_NormalMode_OpensCardURL(t *testing.T) {
	p := provider.NewFakeProvider()
	fe := &action.FakeExecutor{}
	b := NewBoard(p, nil, nil, nil, fe, "matteobortolazzo", "lazyboards", "github", 0, 0, 0, "Working", false, false, nil)
