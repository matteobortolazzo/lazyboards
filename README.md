...
If a card has GitHub's native sub-issue relationships, they render as muted gray status lines above the agent/PR lines: a `󰙅` line shows a parent card's sub-issue count (e.g. `󰙅 3`), and a `󱞩` line shows a child card's parent issue number (e.g. `󱞩 #12`). A card that's both a parent and a child shows both lines, parent first; a card with neither relationship shows no extra line. The full status-line order beneath a card's title is: sub-issue line(s), then agent line(s), then PR line(s).
