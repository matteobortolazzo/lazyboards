...
A card with open GitHub issue-dependency blockers renders a `󰂭` status line in warning-hued orange, naming up to 3 open blockers by number — `owner/repo#N` when the blocker lives in a different repo, or a bare `#N` when it's in the board's own tracked repo — followed by a `+N` remainder for any open blockers beyond the 3 named. This line is width-fitted to the card's available width: at narrower widths it drops named blockers one at a time (rolling each into the `+N` count) until it fits, degrading all the way to just the glyph plus the total `+N` count if nothing else fits. It renders first, ahead of every other status line.

The opposite direction renders too: a card that blocks other open issues shows a `󰳘` line with the count of issues waiting on it (e.g. `󰳘 3`), in the same muted gray as the sub-issue lines rather than the warning hue — other work waiting on this ticket is structural context, not a reason to avoid picking it up. Both counts are GitHub's open-only counts, so a card whose blockers (or whose blocked dependents) have all closed shows no line at all. A card that's both blocked and blocking shows both lines, blocked-by first.

If a card has GitHub's native sub-issue relationships, they render as muted gray status lines above the agent/PR lines: a `󰙅` line shows a parent card's completed/total sub-issue count (e.g. `󰙅 2/3`), and a `󱞫` line shows a child card's parent issue number (e.g. `󱞫 #12`). A card that's both a parent and a child shows both lines, parent first; a card with neither relationship shows no extra line. The full status-line order beneath a card's title is: blocked-by line, then blocking line, then sub-issue line(s), then agent line(s), then PR line(s) — dependency state first, then structure, then activity. Sub-issue lines are gray by design on every card. Agent badges, PR status glyphs, and the per-label color dots next to a card title keep their assigned colors on non-focused cards too — only the title text mutes — preserving cross-card color-coded scanning.
...
| `enter` | `milestone_list.filter` | Toggle filter (remembered per repository across restarts) |
...
Every change to the selection set is saved for the current repository in
`~/.config/lazyboards/state.yml` (see the [remembered state
note](#config-reference)), including the automatic clears when a reference jump
hides its target or a newly created card takes focus. On the next launch the
same set is active before the first fetch. A saved selection that no card
carries any more (a deleted label, say) is kept as saved: it still shows in the
`⚑` segment and `c` removes it. A failed save shows a "Could not save filters"
message and leaves the filter active for the session. Deleting `state.yml`
resets every repository's filters.
