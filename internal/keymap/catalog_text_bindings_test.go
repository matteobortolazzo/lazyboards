...
// directly from the current handler switch statements (mode_handlers.go:
// handleCloseConfirmModeKey :956, handleLabelConfirmModeKey :938,
// handleDeleteModeKey :981, handleCreateModeKey :13, handleConfigModeKey
// :105, handleSearchModeKey :598, handleCommentModeKey :441) -- never from
// defaults_text.go, so a bug in the catalogue/defaults data can't also
// corrupt the expectation, mirroring modalBindingCase (catalog_pr1_bindings_
// test.go). Every binding in these seven modes resolves to a built-in
// command -- none of them dispatches an inline Action -- so unlike
// modalBindingCase there is no wantKind/wantAction field.
...
var textBindingCases = []textBindingCase{
	// close_confirm (handleCloseConfirmModeKey, mode_handlers.go:956)
	...
	// label_confirm (handleLabelConfirmModeKey, mode_handlers.go:938)
	...
	// delete (handleDeleteModeKey, mode_handlers.go:981); delete stays one
	...
	// create (handleCreateModeKey, mode_handlers.go:13)
	...
	// config (handleConfigModeKey, mode_handlers.go:105)
	...
	// search (handleSearchModeKey, mode_handlers.go:598)
	...
	// comment (handleCommentModeKey, mode_handlers.go:441)
