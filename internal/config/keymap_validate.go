...
// it rejects any bound key that is a strict, whitespace-boundary prefix of
// another bound key in the same mode or column -- a prefix key could never
// dispatch, since a pending sequence always waits for a continuation key.
// An explicit `~` unbind on either key removes it from consideration,
...
func validateKeymap(cfg *Config) error {
	...
	for _, mode := range keymap.Modes() {
		if err := validateModePrefixes(resolved, mode, ""); err != nil {
			return err
		}
	}
	...
	if cfg.Keymaps != nil {
		for column := range cfg.Keymaps.Columns {
			for _, mode := range []keymap.Mode{keymap.ModeNormal, keymap.ModeDetail} {
				if err := validateModePrefixes(resolved, mode, column); err != nil {
					return err
				}
			}
		}
...
// validateModePrefixes checks (mode, column)'s resolved, bound (non-
// unbound) entries for a whitespace-boundary prefix conflict: key a
// conflicts with key b when b == a + " " + <more>, mirroring Lookup's own
// pending-match boundary check (lookup.go) so a validated config never
// contains a key that Lookup would report OutcomeNoMatch for a partial
// match that another entry's prefix claims to resolve.
func validateModePrefixes(resolved *keymap.Keymap, mode keymap.Mode, column string) error {
	entries := resolved.Entries(mode, column)
	bound := make([]string, 0, len(entries))
	for _, e := range entries {
		if e.Binding.Kind == keymap.BindingUnbound {
			continue
		}
		bound = append(bound, e.Sequence)
	}

	for i, a := range bound {
		for _, b := range bound[i+1:] {
