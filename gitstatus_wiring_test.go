...
func newGitStatusTestBoard(t *testing.T, reader gitdetect.Reader) Board {
	...
	b := NewBoard(p, nil, nil, nil, "", "", "", 0, 0, "Working", false, false, nil, reader, true)
...
func TestHandleBoardFetched_InitialLoad_TriggersGitStatusRefresh(t *testing.T) {
	p := provider.NewFakeProvider()
	b := NewBoard(p, nil, nil, nil, "", "", "", 0, 0, "Working", false, false, nil, gitdetect.FakeReader{Status: gitdetect.Status{Branch: "main"}}, true)
