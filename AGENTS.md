...
- When adding a new mode or keybinding: status-bar hints and the `?` help modal are both derived from the active `*keymap.Keymap`, so neither is hand-edited. What must be hand-updated is the command catalogue (`internal/keymap/command_*.go`), its default binding (`internal/keymap/defaults_*.go`), and the README Keybindings table for that mode.
